...
building <- fread("Fall2016-Proj2-grp6/data/ManhattanHousing.csv")
food <- fread("Fall2016-Proj2-grp6/data/DOHMH_New_York_City_Restaurant_Inspection_Results.csv")
...
building <- building %>% 
  dplyr::select(NEIGHBORHOOD,CATEGORY = `BUILDING CLASS CATEGORY`,ADDRESS,ZIPCODE = `ZIP CODE`,YEAR = `YEAR BUILT`) %>%
  filter(ZIPCODE > 0) %>%
